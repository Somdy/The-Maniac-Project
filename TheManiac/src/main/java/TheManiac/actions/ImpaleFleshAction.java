...
public class ImpaleFleshAction extends AbstractGameAction {
    ...
    public ImpaleFleshAction(AbstractCreature source, AbstractCreature target, int damage) {
        this.source = source;
        this.target = target;
        this.amount = damage;
            ...
            if (!this.target.isDying && this.target.currentHealth > 0 && !this.target.isEscaping) {
                ...
                if (this.target.lastDamageTaken > 0) {
                    ...
                    if (feedBack > 0) {
                        ...
                        this.source.damage(new DamageInfo(this.source, feedBack, DamageInfo.DamageType.HP_LOSS));
                    }
                }
            }
