...
public class UnseenTerrorPower extends AbstractManiacPower implements CloneablePowerInterface {
    ...
    public UnseenTerrorPower(AbstractCreature owner, int powersAmount) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.target = target;
        ...
        if (this.amount > 0) {
            ...
            for (AbstractMonster monster : AbstractDungeon.getMonsters().monsters) {
                if (!monster.isDeadOrEscaped()) {
                    this.addToBot(new ApplyPowerAction(monster, this.owner, new WeaknessPower(monster, this.amount), this.amount));
                }
            }
        }
    }
    ...
    public AbstractPower makeCopy() {
        return new UnseenTerrorPower(owner, amount);
    }
}
