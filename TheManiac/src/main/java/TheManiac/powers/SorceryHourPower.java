...
public class SorceryHourPower extends AbstractManiacPower implements CloneablePowerInterface {
    public static final String POWER_ID = "maniac:SorceryHourPower";
    ...
    public SorceryHourPower(AbstractCreature owner, int amount) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.type = PowerType.BUFF;
        this.loadImg("SorceryHour");
    ...
    public void onUseCard(AbstractCard card, UseCardAction action) {
        if (this.amount > 0 && (card.costForTurn >= 1 && !card.freeToPlayOnce || card.cost == -1 && card.energyOnUse >= 1)) {
    ...
    public AbstractPower makeCopy() {
        return new SorceryHourPower(owner, amount);
    }
}
